    ...
    try {
        ...
        const ledgerRoomId = await createRoom(client, {
            createOpts: {
                ...
                visibility: Visibility.Private,
                power_level_content_override: {
                    events_default: 0, // All users can read ledger events for balance checking
                    invite: 0,
                    state_default: 100, // Only high-level users can modify ledger state
                    users_default: 0,
                },
...
const SpaceCreateMenu: React.FC<{
    onFinished(): void;
    isDAOMode?: boolean;
}> = ({ onFinished, isDAOMode = false }) => {
    const cli = useMatrixClientContext();
    const [visibility, setVisibility] = useState<Visibility | null>(null);
    const [busy, setBusy] = useState<boolean>(false);
    const [isSpaceMode, setIsSpaceMode] = useState<boolean>(!isDAOMode);
