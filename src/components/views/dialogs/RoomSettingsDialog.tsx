...
import DCARoomSettingsTab from "../settings/tabs/room/DCARoomSettingsTab";
import { findDCASpace } from "../../../utils/DCASettings";
...
class RoomSettingsDialog extends React.Component<IProps, IState> {
    ...
    private getTabs(): NonEmptyArray<Tab<RoomSettingsTab>> {
        ...
        // Add DCA settings tab if this room is part of a DCA space
        const dcaSpace = findDCASpace(this.state.room);
        if (dcaSpace) {
            tabs.push(
                new Tab(
                    RoomSettingsTab.DCA,
                    _td("DCA Settings"),
                    "mx_RoomSettingsDialog_dcaIcon",
                    <DCARoomSettingsTab room={this.state.room} />,
                ),
            );
        }
