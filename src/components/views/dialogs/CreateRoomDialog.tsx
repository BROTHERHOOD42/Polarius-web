...
export default class CreateRoomDialog extends React.Component<IProps, IState> {
        ...
        opts.roomType = this.props.type;
         ...
         if (this.isGOVSpace()) {
             if (this.state.agendaType === 'proposal') {
                 // Use the name directly without prefix
        ...
        if (this.state.joinRule === JoinRule.Public) {
         ...
         if (this.state.topic) {
             if (this.isDCASpace() && this.state.contributionValue) {
                 ...
                 // For GOV space, add proposal marker and truncate description to 150 characters for room topic
                 const truncatedTopic = this.state.topic.length > 150 
                     ? this.state.topic.substring(0, 150) + "..."
                     : this.state.topic;
                 
                 if (this.state.agendaType === 'proposal') {
                     createOpts.topic = `[GOV_PROPOSAL]\n${truncatedTopic}`;
                 } else {
                     createOpts.topic = `[GOV_DISCUSSION]\n${truncatedTopic}`;
                 }
             } else {
                 createOpts.topic = this.state.topic;
             }
    ...
    private onFormSubmit = (ev: SyntheticEvent): void => {
        // Prevent form submission on Enter key press
        ev.preventDefault();
        ev.stopPropagation();
        // Do nothing - user must click the "Create Proposal" button
        console.log("Form submit prevented - user must click Create Proposal button");
    };
                            ...
                            <div className="mx_CreateRoomDialog_section mx_CreateRoomDialog_votingSystem">
                                ...
                                <div className="mx_CreateRoomDialog_votingSystem_content">
                                    ...
                                    <div className="mx_CreateRoomDialog_votingSystem_info">
                                        ...
                                        {this.state.votingSystem.duration === 10 
                                            ? `Basic voting with For and Against or Abstain options. Voting will be open for 10 seconds.`
