...
function isGOVProposalRoom(room: Room): boolean {
    // Method 1: Check if this room is a child of GOV space using SpaceStore
    const parentSpaces = SpaceStore.instance.getParents(room.roomId);
    const hasGOVParent = parentSpaces.some((parent: any) => {
        const parentRoom = room.client.getRoom(parent.roomId);
        return parentRoom && parentRoom.name === "GOV";
    });
    
    if (hasGOVParent) {
        console.log(`✅ [VOTING POWER] Found GOV parent via SpaceStore for room: ${room.name}`);
        return true;
    }
    
    // Method 2: Search all spaces for GOV space and check if this room is its child
    console.log(`🔍 [VOTING POWER] No GOV parent found via SpaceStore, searching all spaces...`);
    const allSpaces = SpaceStore.instance.spacePanelSpaces;
    
    for (const space of allSpaces) {
        const spaceRoom = room.client.getRoom(space.roomId);
        if (spaceRoom && spaceRoom.isSpaceRoom() && spaceRoom.name === "GOV") {
            console.log(`🔍 [VOTING POWER] Found GOV space: ${spaceRoom.name} (${spaceRoom.roomId})`);
            
            // Check if this room is a child of the GOV space
            const children = SpaceStore.instance.getChildren(spaceRoom.roomId);
            const isChild = children.some(child => child.roomId === room.roomId);
            
            console.log(`🔍 [VOTING POWER] Room ${room.roomId} is child of GOV space: ${isChild}`);
            if (isChild) {
                console.log(`✅ [VOTING POWER] Found GOV parent via search for room: ${room.name}`);
                return true;
            }
        }
    }
    
    console.log(`❌ [VOTING POWER] No GOV parent found for room: ${room.name}`);
    return false;
}
